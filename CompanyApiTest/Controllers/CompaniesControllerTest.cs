...
namespace CompanyApiTest.Controllers
{
    public class CompaniesControllerTest
    {
        ...
        {
            testClient = new TestServer(new WebHostBuilder().UseStartup<Startup>()).CreateClient();
        }
        ...
        public async Task Should_Create_A_New_Company_When_Post_A_New_Company()
        {
            ...
            // when
            var response = await testClient.PostAsync("/companies", requestBody);
        ...
        public async Task Should_Not_Create_Company_When_Post_An_Existed_Company()
        {
            ...
            var secondResponse = await testClient.PostAsync("/companies", secondRequestBody);
            // then
        ...
        public async Task Should_Return_All_Company_List_When_Get_GetAllCompany()
        {
            ...
            string request = JsonConvert.SerializeObject(company);
            StringContent requestBody = new StringContent(request, Encoding.UTF8, "application/json");
            await testClient.PostAsync("/companies", requestBody);
            // when
            var response = await testClient.GetAsync("companies");
            // then
            response.EnsureSuccessStatusCode();
            var responseString = await response.Content.ReadAsStringAsync();
        ...
        public async Task Should_Return_Specified_Company_When_Get_GetCompanyByID()
        {
            ...
            await testClient.PostAsync("/companies", requestBody);
            // when
            var response = await testClient.GetAsync("companies/0");
            // then
            response.EnsureSuccessStatusCode();
            var responseString = await response.Content.ReadAsStringAsync();
        ...
        [Fact]
        public async Task Should_Return_Page_Size_Company_From_StartPage_When_Get_GetAllCompany_By_Page()
        {
            // given
            await testClient.DeleteAsync("companies/clear");
            ...
            await testClient.PostAsync("/companies", requestBody2);
            ...
            await testClient.PostAsync("/companies", requestBody3);
            // when
            var response = await testClient.GetAsync("companies?pagesize=2&startpage=2");
            // then
            response.EnsureSuccessStatusCode();
            var responseString = await response.Content.ReadAsStringAsync();
            var actualCompanies = JsonConvert.DeserializeObject<List<Company>>(responseString);
        ...
        public async Task Should_Created_New_Employee_When_Post_AddEmployee()
        {
            ...
            StringContent requestBody = new StringContent(request, Encoding.UTF8, "application/json");
            var response = await testClient.PostAsync("/companies/0/employees", requestBody);
            // then
            response.EnsureSuccessStatusCode();
            var getResponse = await testClient.GetAsync("companies/0");
            var getResponseString = await getResponse.Content.ReadAsStringAsync();
        ...
        public async Task Should_Return_All_Employee_When_Get_GetAllEmployee()
        {
            ...
            await testClient.PostAsync("/companies", requestBody1);
            // when
            var getResponse = await testClient.GetAsync("companies/0/employees");
            var getResponseString = await getResponse.Content.ReadAsStringAsync();
        ...
        [Fact]
        public async Task Should_Delete_Specified_Employee_In_Specified_Company_When_Delete_DeleteEmployee()
        {
            // given
            await testClient.DeleteAsync("companies/clear");
            Company company1 = new Company("0", "Baymax");
            ...
            await testClient.PostAsync("/companies", requestBody1);
            // when
            await testClient.DeleteAsync("/companies/0/employees/0");
            // then
            var getResponse = await testClient.GetAsync("companies/0");
            var getResponseString = await getResponse.Content.ReadAsStringAsync();
            var actualCompany = JsonConvert.DeserializeObject<Company>(getResponseString);
            Assert.Empty(actualCompany.Employees);
        }

        [Fact]
        public async Task Should_Delete_Specified_Company_When_Delete_DeleteCompany()
        {
            // given
            await testClient.DeleteAsync("companies/clear");
            Company company1 = new Company("0", "Baymax");
            ...
            await testClient.PostAsync("/companies", requestBody1);
            // when
            await testClient.DeleteAsync("/companies/0");
            // then
            var getResponse = await testClient.GetAsync("companies");
            var getResponseString = await getResponse.Content.ReadAsStringAsync();
            var actualCompany = JsonConvert.DeserializeObject<List<Company>>(getResponseString);
            Assert.Empty(actualCompany);
        }
    }
}
